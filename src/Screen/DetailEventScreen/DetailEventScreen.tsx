...
const DetailtEventScreen = ({ route }: { route: any }) => {
  ...
  const getDetailEvent = () => {
    axios
      .get(`${BASE_URL}/api/event-detail/${route.params.id}`, {
        ...
        <>
          <Box mt={5} mx={5}>
            <Text fontSize={16} mb={2} fontWeight="bold">
              Daftar List Keluarga
            </Text>
            <Checkbox.Group
              onChange={setListDaftarKeluarga}
              value={listDaftarKeluarga}>
              {userInfo.user.pasangan != null ? (
                <Checkbox value={userInfo.user.pasangan.id}>
                  {userInfo.user.pasangan.name}
                </Checkbox>
              ) : (
                <></>
              )}
              {userInfo.anak.length != 0 ? (
                userInfo.anak.map((anak, index) => {
        ...
        <>
          <Box mt={5} mx={5}>
            ...
            <Checkbox.Group
              onChange={setListDaftarKeluarga}
              value={listDaftarKeluarga}>
              {userInfo.eyang.length != 0 ? (
                userInfo.eyang.map((eyang, index) => {
                  return <Checkbox value={eyang.id}>{eyang.name}</Checkbox>;
                })
              ) : (
                <></>
              )}
              {userInfo.orangtua.length != 0 ? (
                userInfo.orangtua.map((orangTua, index) => {
                  return (
                    <Checkbox value={orangTua.id}>{orangTua.name}</Checkbox>
                  );
                })
              ) : (
                <></>
              )}
              {userInfo.saudara.length != 0 ? (
                userInfo.saudara.map((saudara, index) => {
      ...
      <>
        ...
        {isLoading ? (
          ...
          eventDetail.event_media[0].jenis == 'image' ? (
            <ImageBackground
              source={{
                uri: `${BASE_URL}/storage/files/event-media/${eventDetail.event_media[0].file}`,
              }}
              resizeMode="cover"
              style={styles.image}
            />
              ...
              {isLoading ? (
                ...
                eventDetail.event_media.map((item, index) =>
                  item.jenis == 'image' ? (
                    <Image
                      source={{
                        uri: `${BASE_URL}/storage/files/event-media/${item.file}`,
                      }}
                      alt="Alternate Text"
                    ...
                    <TouchableOpacity
                      onPress={() => {
                        Linking.openURL(`https://youtu.be/${item.file}`);
                      }}>
                      <Box mr={3}>
                        <ImageBackground
                    ...
                    <TouchableOpacity
                      onPress={() => {
                        Linking.openURL(
                          `https://open.spotify.com/episode/${item.file}`,
                        );
                      }}>
                      <Box mr={3}>
                        <ImageBackground
        ...
        <Box mx={5} flex={1}>
          <Text fontWeight="bold" color="#555F65" fontSize={16}>
            Description
          </Text>
          <RenderHTML
            contentWidth={width}
          ...
          <Box mt={5}>
            {isLoading ? (
              ...
              <TouchableOpacity
                onPress={() => {
                  Linking.openURL(
                    `geo:0.05,0.05?q=${eventDetail.lat},${eventDetail.lng}`,
                  );
                }}>
                <MapView
        ...
        {isLoadingInfoDaftar ? (
          ...
          <Text mx={5} mt={5} color="#91989F" textAlign="justify" fontSize={12}>
            note : {noteMessage}
          </Text>
        ) : (
          <></>
        ...
        {eventDetail.status == 'finish' ? (
          ...
          <>
            ...
            {mendaftarKeluarga ? (
              <>
                <ButtonDaftarKeluarga />
                <TouchableOpacity onPress={() => setMendaftarKeluarga(false)}>
                  <Box
                    bgColor="#032844"
                    borderRadius={50}
                    py={3}
              ...
              <TouchableOpacity onPress={() => setMendaftarKeluarga(true)}>
                <Box
                  bgColor="#032844"
                  borderRadius={50}
                  py={3}
